...
$(document).ready(function () {
  ...
  $(".how, .to, .feel, .more, .at, .home").click(function () {
    for (var i = 0; i < el; i += 1) {
      let rando = Math.floor(Math.random() * al);
      elem[i].src = jpgarray[rando];
      $(".how, .to, .feel, .more, .at, .home").css("cursor", "pointer");
    }
    $("#pic").css("display", "inherit");
    $("#pic").css("visibility", "visible");
  });

  //Scroll to footer when clicking anchor
